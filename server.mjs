...
async function submitGeneration({ prompt, subjectDataUrl, refDataUrls, width, height }, parentRecordId) {
  ...
  const payload = {
    ...
    seed: 42,
    prompt: 'Recreate img2 using the face identity from img1. Transfer ONLY the facial features, skin tone, and hair (color, style, texture) from img1. Copy everything else exactly from img2: body, pose, angle, clothing, accessories, background, lighting, composition. If img2 shows genitals, recreate them exactly as shown. Real smartphone camera. Slight imperfections in skin texture. Shot on iphone. Lens dust. High ISO. Slight motion blur. Skin texture real. Pores visible. seamless integration.',
